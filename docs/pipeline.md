...
`assets/` holds the converted game data — about 23 MB, and only what is
actually loaded. All but the planet is converted Nintendo game data; the planet
is generated by this project and carries none of it:
...
`assets/bevy/planet.glb` is a copy, and the `planet` stage of
`build_assets.py` is a copy step rather than a build. The generator is its own
program with its own Blender pass — see
[`experimental/planet_gen/readme.md`](../experimental/planet_gen/readme.md) —
and running it from here would mean this pipeline owning a planet's worth of
parameters it has no opinion about. Regenerate the source with, from
...
then `python3 tools/build_assets.py --only planet` to adopt it.

LOD0 is what the game takes, and there is no second file: the collision the
player stands on is read out of this same mesh as it loads. A LOD1 planet would
be a planet whose ground is a few metres from where it is drawn. There is no
