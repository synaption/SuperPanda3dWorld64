...
A working planet. `planetgen build` reads six face rasters and writes 96 LOD0
tiles plus 6 LOD1 tiles in about a second; the Blender export takes another
three and writes `out/planet.blend` alongside `out/planet.glb` (LOD0) and
`out/planet_lod1.glb` (LOD1). Renders are in `out/`.
...
python3 -m unittest discover -s tests        # 16 tests, no Blender needed
...
| Triangles | 786,432 across 96 LOD0 tiles |
...
- LOD skirts, and the Render96 triplanar materials. Tiles carry a material
  index and a flat vertex colour for now.
...
rather than accumulating copies. Tiles are separate objects, not one joined
...
- **Water.** [`src/level.rs`](../../src/level.rs) does water as axis-aligned
  boxes, which does not survive being wrapped onto a sphere. A planet ocean is
  a sphere at sea level; a lake on a slope is neither. Undesigned, and visible
  in `out/planet_space.png` as bare grey ocean floor -- the basins are correct
  terrain with nothing drawn on top of them.
